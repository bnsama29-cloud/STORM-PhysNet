...
To isolate our physics components, we evaluate several alternatives against STORM-BzGate. The Vanilla Transformer uses the same attention core and heads but lacks the delay and gating, achieving $\mathrm{PE}_{\mathrm{all}}=0.648\pm0.031$ but suffering instability at 45 minutes. Standard LSTM and MLP models offer intermediate accuracy, while a CNN with global pooling fails on this dataset. We also test gating variants—STORM-Cathode (bipolar $B_z$/$P_{\mathrm{dyn}}$), STORM-Radiotrophic (amplitude only), and STORM-Cathode+Spec (spectral residual)—all sharing the identical delay and Transformer core. None match BzGate's storm-time performance. A magnitude-only gate (BzMag) collapses completely, proving polarity is essential. Table~\ref{tab:main} reports all variants, confirming BzGate as the optimal model.
%% ====================================================================
\section{Results}
%% ====================================================================
...
Cathode, Radiotrophic, and Cathode+Spec variants in Table~\ref{tab:main} are modular gating modifications defined in Section~IV, not distinct end-to-end models.
