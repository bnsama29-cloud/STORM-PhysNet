...
Table~\ref{tab:main} summarizes fifteen-seed test results. On all-hour 6\,h PE, the Transformer baseline is $0.895$.
...
Bootstrap 95\% confidence intervals for PE$_{1\mathrm{h}}$
...
Transformer at inference. We recommend bagging operationally due to this computational simplicity, as both approaches perform similarly at 6\,h and 12\,h.
...
Ensemble (\(\alpha^*=0.3\))  & \(0.983\) & \(\mathbf{0.911}\) & \(\mathbf{0.870}\) & \(0.839\) \\
...
Relative to a strong Transformer, the clearest single-model gain is short-horizon
