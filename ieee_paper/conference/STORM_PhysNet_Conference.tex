...
0.740 to 0.841 (paired \(p = 7.6\times10^{-7}\)) and 12\,h PE from
0.567 to 0.762 (\(p = 3.6\times10^{-10}\)) across fifteen seeds.
...
\caption{STORM-PhysNet data flow: OMNI solar wind and GOES flux enter a learnable
L1--Earth delay and a temporal Transformer encoder ($d_{\mathrm{model}}=128$,
two layers, four heads). A $B_z$-conditioned gate modulates the representation;
three residual heads emit simultaneous 1\,h, 6\,h, and 12\,h log-flux forecasts.}
\label{fig:arch}
\end{figure*}
...
The mixing weight \(\alpha\) was selected on the validation split. 
At the selected \(\alpha^*\), test PE reaches \(0.906\) at 6\,h and \(0.862\) at 12\,h.
...
Transformer bagged           & \(0.984\) & \(0.907\) & \(0.861\) & \(0.816\) & --- \\
\bottomrule
\end{tabular}
...
\(0.74\) and \(0.56\). Fine-tuning on GRASP (freezing the Transformer encoder and updating only the forecast heads) raises 6\,h PE from \(0.740 \pm 0.020\) to \(0.841 \pm 0.017\) (paired \textit{t}-test across fifteen seeds, \(p = 7.6\times10^{-7}\), Cohen's \(d = 2.17\)) and 12\,h PE from \(0.567 \pm 0.033\) to \(0.762 \pm 0.020\) (\(p = 3.6\times10^{-10}\), \(d = 3.98\)). Absolute GRASP PE remains below
