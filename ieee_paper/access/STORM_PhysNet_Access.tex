...
\tfootnote{This work extends a concurrent conference submission on the same system. The present manuscript adds the wider-delay ablation, the bagged-Transformer control, the capacity-matched Transformer control, expanded interpretability diagnostics, and the full multi-seed GRASP transfer analysis.}
\markboth
...
On hourly GOES--OMNI data with fifteen random initializations of the same chronological split, STORM-Bz improves short-horizon reliability (PE$_{45\mathrm{min}}=0.986$ versus Transformer $0.977$, paired $p = 0.002$) and remains clearly positive against persistence (PE$\approx0.32$ versus $\approx-0.14$). At 6\,h and 12\,h a plain Transformer remains competitive or slightly stronger as a single model. A validation-selected linear ensemble ($\alpha^*=0.3$) and seed-bagged STORM both reach 6\,h PE of $0.911$. A capacity-matched Transformer control ($d_{\mathrm{model}}=128$, two layers, four heads) reached mean PE$_{45\mathrm{min}}=0.981$ and PE$_{6\mathrm{h}}=0.907$ across fifteen seeds; bagging those seeds yields $0.986$ / $0.919$ / $0.877$, establishing the bagged capacity-matched Transformer as the strongest model at the 6\,h and 12\,h horizons. While STORM dominates the single-model 45-min forecast ($0.988$ bagged), capacity and bagging move multi-horizon PE more than the physics modules at inference. Controlled ablations show that the short-horizon gain is a property of the trained package as a whole rather than of any individual physics module at inference.
...
they are not capacity-matched. We additionally trained a capacity-matched
...
to STORM in width or depth. We also trained a capacity-matched Transformer
control ($d_{\mathrm{model}}=128$, 2 layers, 4 heads; $\approx1.19\times10^{6}$
parameters) under the same fifteen seeds; its PE is reported separately in
Table~\ref{tab:main}. The comparison should be interpreted with 
...
competitive on all-hour 6\,h PE and improving short-horizon reliability. A capacity-matched
...
A capacity-matched Transformer control ($d_{\mathrm{model}}=128$, two layers)
...
remains below the GOES test scores, as expected under sensor and longitude shift. A similar adaptation profile is observed for the capacity-matched Transformer baseline, which improves its 6\,h PE from \(0.549 \pm 0.015\) (zero-shot) to \(0.625 \pm 0.021\) after fine-tuning.
...
The default Transformer baseline is not capacity-matched to STORM; the matched
...
against persistence at 45\,min). The clearest multi-horizon gain among the systems evaluated here comes from a capacity-matched bagged Transformer, which reaches test PE\(_{6\mathrm{h}}=0.919\) and PE\(_{12\mathrm{h}}=0.877\). A linear ensemble with validation-selected \(\alpha^*=0.3\) and seed-bagged STORM also achieve strong multi-horizon results (both reaching PE\(_{6\mathrm{h}}=0.911\)). Input-noise and missing-channel checks, together with GRASP fine-tuning
gains at 6--12\,h, support operational motivation beyond a pure leaderboard PE
number; for instance, the capacity-matched baseline achieves a marginal persistence prediction efficiency (PE\(_{\mathrm{pers}}\)) of \(0.053 \pm 0.148\) at 45 minutes, illustrating the difficulty of solidly outperforming operational persistence models. Furthermore, under synthetic Gaussian noise injection (\(\sigma=1.0\)), the 6\,h PE of the matched baseline drops to \(0.825 \pm 0.011\), highlighting the need for structurally robust architectures during degraded sensor conditions. Horizon-restricted physics losses did not improve PE in a fifteen-seed
ablation.
