...
STORM-Bz improves short-horizon reliability
(PE$_{1\mathrm{h}}=0.986$ versus Transformer $0.978$)
and remains clearly positive against persistence
(PE$_{\mathrm{pers}}\approx0.31$ versus $\approx-0.08$).
At 6\,h and 12\,h, single-model Transformer and STORM-Bz are close
(PE$_{6\mathrm{h}}=0.895$ vs.\ $0.900$).
Seed-bagged STORM-Bz reaches PE$_{6\mathrm{h}}=0.910$ and PE$_{12\mathrm{h}}=0.870$.
An architecture-matched Transformer control
($d_{\mathrm{model}}=128$, two layers, four heads)
reaches mean PE$_{1\mathrm{h}}=0.980$ and PE$_{6\mathrm{h}}=0.895$;
bagging yields $0.984$ / $0.908$ / $0.861$.
Controlled ablations show that the short-horizon gain is a property of the trained package
as a whole rather than of any individual physics module at inference.
...
\midrule
STORM-Bz (physics \(B_z\)) & 0.986 & 0.900 & 0.854 \\
...
\midrule
LSTM & \(0.955\) & \(0.881\) & \(0.840\) & --- \\
Transformer & \(0.978\) & \(0.895\) & \(0.845\) & --- \\
STORM-Bz & \(0.986\) & \(0.900\) & \(0.854\) & --- \\
STORM (No-Delay) & \(0.987\) & \(0.902\) & \(0.855\) & --- \\
STORM (No-Physics) & \(0.986\) & \(0.899\) & \(0.850\) & --- \\
STORM (No-Gate) & \(0.986\) & \(0.900\) & \(0.856\) & --- \\
Ensemble \(\alpha^*=0.3\)\textsuperscript{a} & \(0.983\) & \(\mathbf{0.911}\) & \(\mathbf{0.870}\) & \(0.839\) \\
STORM bagged (15 seeds) & \(\mathbf{0.987}\) & \(\mathbf{0.910}\) & \(\mathbf{0.870}\) & \(0.849\) \\
Transformer matched (mean) & \(0.980\) & \(0.895\) & \(0.845\) & --- \\
...
where computed over seeds. \textsuperscript{a}Ensemble rows are single aggregate evaluations (\(\alpha^*\) applied once); bootstrap CIs are therefore not reported for these rows. Matched-Transformer rows are not bootstrapped because bagged evaluation uses a single aggregate prediction set.} \\
\end{tabular}
\end{table*}

On all-hour 6\,h PE, the Transformer baseline is \(0.895\).
A single STORM-Bz model is comparable (\(0.900\)) and is better at 1\,h
(\(0.986\) versus \(0.978\), \(p = 0.002\), Cohen's \(d\approx0.98\)).
Seed-bagged STORM reaches PE\(_{6\mathrm{h}}=0.910\) and PE\(_{12\mathrm{h}}=0.870\).
Bootstrap 95\% confidence intervals for PE\(_{1\mathrm{h}}\) were selected on the validation split by
...
transfer path toward GSAT-19 GRASP. Relative to a strong Transformer, the clearest single-model gain is short-horizon
...
(PE$_{6\mathrm{h}}\approx0.910$--$0.911$, PE$_{12\mathrm{h}}\approx0.870$).
...
number; for instance, the architecture-matched baseline achieves a marginal persistence prediction efficiency (PE\(_{\mathrm{pers}}\)) of \(0.053 \pm 0.148\) at 1\,h, illustrating the difficulty of solidly outperforming operational persistence models. Furthermore, under synthetic Gaussian noise injection (\(\sigma=1.0\)), the 6\,h PE of the matched baseline drops to \(0.825 \pm 0.011\), highlighting the need for structurally robust architectures during degraded sensor conditions. GRASP transfer and noise-robustness figures follow the prior checkpoint campaign unless noted; the full Kaggle retrain refreshed the GOES multi-seed PE tables. Horizon-restricted physics losses did not improve PE in a fifteen-seed
ablation.
