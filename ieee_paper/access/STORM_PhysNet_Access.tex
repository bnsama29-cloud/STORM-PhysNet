...
the broad shape of the multi-day enhancement and recovery more closely than persistence. We do not claim the gate panel tracks reconnection from these plots alone.
...
Unless noted, all GOES numbers below are PE{\mathrm{clim}}$ on the fixed
...
\begin{table*}[!t]
\centering
\caption{GOES test PE{\mathrm{clim}}$ (seed mean). PE{\mathrm{pers},1\mathrm{h}}$ is
skill versus persistence; PE{\mathrm{st},6\mathrm{h}}$ is restricted to storm-flagged
...
\toprule
System & PE{1\mathrm{h}}$ & PE{6\mathrm{h}}$ & PE{12\mathrm{h}}$ & PE{\mathrm{st},6\mathrm{h}}$ & PE{\mathrm{pers},1\mathrm{h}}$ \\
\midrule
...
Table~\ref{tab:main} is the primary GOES leaderboard for this paper.
The largest single-model separation versus the default Transformer is at 1\,h
(PE{\mathrm{clim}}$ and especially PE{\mathrm{pers}}$). At 6--12\,h, seed means
...
\caption{Per-horizon PE{\mathrm{clim}}$ (seed mean $\pm$ std) for the main systems
...
STORM-Bz \(0.812\), LSTM \(0.827\), Transformer \(0.821\), with
...
\(validation-selected \alpha^* (typically 0.4--0.6); \alpha=0.3 is a fixed diagnostic\) ensemble reaches \(0.839\) and seed-bagged STORM
...
\midrule
LSTM & .47\times10^{4}$ \\
STORM-Bz / No-Delay / No-Gate / No-Physics & .43\times10^{5}$ \\
RDG (cathode--anode gate) & .41\times10^{5}$ \\
RDG-S (RDG + spectral head) & .51\times10^{5}$ \\
Transformer (default) & .45\times10^{5}$ \\
Transformer matched ({=}128$, 2 layers) & .19\times10^{6}$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:params} shows that the default Transformer is wider/deeper than STORM
...
Beyond the default $ gate we evaluated three probes under the same
...
gating nonlinearity, not as replacements for the default $ gate in the main claim.

\begin{table}[!t]
\centering
\caption{Alternative gates (seed-mean PE{\mathrm{clim}}$, fifteen seeds).}
\label{tab:alt_gates}
\begin{tabular}{lccc}
\toprule
Gate & PE{1\mathrm{h}}$ & PE{6\mathrm{h}}$ & PE{12\mathrm{h}}$ \\
\midrule
...
\caption{Alternative gates at 6\,h (mean $\pm$ seed std). Differences relative to
STORM-Bz are small compared with seed scatter.}
\label{fig:alt_gates}
\end{figure}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_physics_gate_storm_quiet.png}
...
short-horizon PE near \(0.82\), but 6\,h and 12\,h PE fall. Fine-tuning on GRASP (freezing the Transformer encoder and updating only the forecast heads) raises 6\,h PE from 0.740 to 0.841 (paired \textit{t}-test across fifteen seeds, \(p = 7.6\times10^{-7}\), Cohen's \(d = 2.17\)) and 12\,h PE from 0.567 to 0.762 (\(p = 3.6\times10^{-10}\), \(d = 3.98\)).
...
Seed-bagged STORM reaches a multi-horizon PE level comparable to the linear ensemble (0.906 at 6\,h).
...
Alternative gates (e.g. RDG, SDG) are detailed in Table~\ref{tab:alt_gates}.
...
(PE$_{6\mathrm{h}}=0.910$, PE$_{12\mathrm{h}}\approx0.870$).
