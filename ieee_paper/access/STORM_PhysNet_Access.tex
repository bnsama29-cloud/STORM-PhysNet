...
On hourly GOES--OMNI data with fifteen random initializations of the same chronological split, STORM-Bz improves short-horizon reliability (PE$_{1\mathrm{h}}=0.986$ versus Transformer $0.977$, paired $p = 0.002$) and remains clearly positive against persistence (PE$\approx0.32$ versus $\approx-0.14$). At 6\,h and 12\,h a plain Transformer remains competitive or slightly stronger as a single model. A validation-selected linear ensemble ($\alpha^*=0.3$) and seed-bagged STORM both reach 6\,h PE of $0.911$. An architecture-matched Transformer control ($d_{\mathrm{model}}=128$, two layers, four heads) reached mean PE$_{1\mathrm{h}}=0.981$ and PE$_{6\mathrm{h}}=0.907$ across fifteen seeds; bagging those seeds yields $0.986$ / $0.919$ / $0.877$, establishing the bagged architecture-matched Transformer as the strongest model at the 6\,h and 12\,h horizons. While STORM dominates the single-model 1-hour forecast ($0.988$ bagged), encoder width/depth and bagging move multi-horizon PE more than the physics modules at inference. Controlled ablations show that the short-horizon gain is a property of the trained package as a whole rather than of any individual physics module at inference.
...
a strong Transformer (significant PE\(_{\mathrm{clim}}\) gain at 1\,h;
...
simultaneous 1-hour, 6-hour, and 12-hour log-flux forecasts.}
\label{fig:arch}
\end{figure*}
...
\boldsymbol{\sigma}_{\mathrm{raw}} &= \mathrm{MLP}(\mathbf{u}_{B_z})\in(0,1)^{d_{\mathrm{model}}} \\
\mathbf{g} &= g_{\mathrm{min}} + (1 - g_{\mathrm{min}})\boldsymbol{\sigma}_{\mathrm{raw}} \\
\mathbf{h}_{\mathrm{gated}} &= \mathbf{g} \odot \mathbf{h}
\label{eq:gate_clamp}
\end{align}
...
Training minimises a composite objective: a primary multi-horizon residual loss (beta-NLL with a \(2.5\times\) under-prediction penalty during storm periods), plus auxiliary Dst/Kp and storm-onset terms and small regularisers (monotonicity, temporal smoothness, delay, variance calibration, and an Akasofu-\(\varepsilon\) coupling consistency term). Horizon weights emphasise the short horizon. Coefficient values follow the released configuration. We did not ablate every auxiliary term individually; reported PE reflects the full composite objective used in training.

\subsection{Supplementary Gate Nonlinearities}
\label{sec:supp-gates}

The main STORM-Bz configuration uses a physics-motivated $B_z$ gate whose
inputs are southward IMF duration and related solar-wind features. To test
whether PE depends on that specific nonlinearity---or only on the shared
training package (storm-biased sampling, residual heads, multi-horizon
loss)---we trained three alternative gates that keep the same Transformer
backbone, delay module, residual heads, data splits, and fifteen seeds
$\{42,\ldots,56\}$. Only the gate block (and, for one variant, an auxiliary
head) changes. Internal code keys differ from the names below; see the
repository map in Sec.~\ref{sec:supp-gates-map}.

\paragraph{Rectified Driver Gate (RDG).}
RDG is a circuit-inspired \emph{shape} prior, not a claim that GEO electrons
behave like a vacuum tube. Solar-wind driving is treated as an emission-like
control signal: a learned estimator of driver strength (built from $v_{\mathrm{sw}}$
and $B_z$) is passed through a rectified, smooth nonlinearity so that only
one polarity of driving strongly opens the gate. The gated representation
fed to the residual heads is
\[
  h_{\mathrm{g}} = g_{\mathrm{RDG}}(x_{\mathrm{sw}})\,\odot\,h,
\]
where $h$ is the encoder state and $g_{\mathrm{RDG}}\in(0,1]$ is the gate
activation. Relative to the physics $B_z$ gate, RDG emphasizes
\emph{half-wave / driver-aligned} modulation rather than an explicit
southward-duration feature path.

\paragraph{RDG with Spectral Head (RDG-S).}
RDG-S uses the same RDG block and adds a small auxiliary head that predicts
spectral parameters $(\kappa,\theta)$ and maps them to an optional
$\ge2\,\mathrm{MeV}$ log-flux estimate. The primary multi-horizon loss still
trains the residual flux heads; the spectral branch is additive and does not
replace $\hat{y}$. This tests whether an explicit spectral parameterization
improves PE when the gate is already non-physics-$B_z$.

\paragraph{Saturating Dose Gate (SDG).}
SDG is a dose-response \emph{shape} prior: recent energy-related input is
accumulated with a learnable scale and passed through a saturating curve
(smooth approach to a ceiling), so the gate opens more under sustained
forcing and levels off rather than growing without bound. This is an
accumulation-then-saturation inductive bias, not a biological model of the
radiation belts. Formally, if $d_t$ denotes a learned scalar summary of recent
driving,
\[
  g_{\mathrm{SDG}} = g_{\min} + (1-g_{\min})\,\sigma(\alpha d_t+\beta),
\]
with learnable $(\alpha,\beta)$ and a floor $g_{\min}$ to avoid hard zeros.

\paragraph{Protocol and results.}
Each variant was trained for up to 40 epochs with the same optimizer,
early-stopping rule, and PE$_{\mathrm{clim}}$ evaluation as STORM-Bz.
Table~\ref{tab:supp-gates} summarizes mean PE over fifteen seeds.
All three variants remain near PE$_{1\mathrm{h}}=0.987$ and
PE$_{6\mathrm{h}}\approx0.91$. Pairwise differences among RDG, RDG-S, and SDG
are within seed noise. None provides a consistent multi-horizon gain that
would justify replacing the physics $B_z$ gate as the default reported system.
We therefore keep STORM-Bz as the primary model for interpretability
(gate activation vs.\ $B_z$/Dst diagnostics) and treat RDG / RDG-S / SDG as
evidence that short-horizon skill is largely a property of the shared training
recipe rather than of one particular gate formula.

\begin{table}[!t]
\centering
\caption{Supplementary gates (fifteen seeds). Mean PE$_{\mathrm{clim}}$ on the
fixed chronological GOES test split. STORM-Bz from the main table is repeated
for reference.}
...
\paragraph{Repository name map.}
\label{sec:supp-gates-map}
For reproducibility, folder and \texttt{gate\_type} strings in the code are
unchanged: RDG $\leftrightarrow$ \texttt{cathode\_anode} /
\texttt{storm\_cathode}; RDG-S $\leftrightarrow$
\texttt{storm\_cathode\_spec}; SDG $\leftrightarrow$
\texttt{radiotrophic} / \texttt{storm\_radiotrophic}. Paper text uses only
RDG, RDG-S, and SDG.

\subsection{Baselines and Training}
We compare against Vanilla Transformer (same residual heads, no delay/gate), a
Transformer baseline (default hyperparameters, not matched in width or depth to the STORM encoder), and LSTM. Training uses Adam~\cite{b28} with initial learning
rate \(3\times10^{-4}\), cosine decay, batch size 64, and early stopping on
12\,h validation MSE. Primary STORM and Transformer runs use seeds \(\{42,\ldots,56\}\).
...
ignored by the optimizer. Early stopping monitors the 12\,h validation MSE rather than
...
We also train a Transformer with the \emph{same encoder hyperparameters} as STORM ($d_{\mathrm{model}}=128$, two layers, four heads; $\approx1.19\times10^{6}$ parameters). This architecture-matched control is larger in parameter count than STORM and is not a parameter-matched baseline; the default Transformer uses $d_{\mathrm{model}}=64$, three layers ($\approx8.45\times10^{5}$ parameters). LSTM remains a useful sequential control because much of the older
...
\begin{tabular}{lcccc}
\toprule
System & PE\(_{1\mathrm{h}}\) & PE\(_{6\mathrm{h}}\) & PE\(_{12\mathrm{h}}\) & PE\(_{\mathrm{st},6\mathrm{h}}\) \\
\midrule
LSTM
...
Transformer matched (mean)
  & \(0.981\) & \(0.907\) & \(0.861\) & --- \\
Transformer matched (bagged)
  & \(0.986\) & \(\mathbf{0.919}\) & \(\mathbf{0.877}\) & --- \\
\bottomrule
...
where computed over seeds. \textsuperscript{a}Ensemble and Hybrid rows are single aggregate evaluations (\(\alpha^*\) applied once); bootstrap CIs are therefore not reported for these rows. Matched-Transformer rows are not bootstrapped because bagged evaluation uses a single aggregate prediction set.} \\
\end{tabular}
\end{table*}

On all-hour 6\,h PE, the Transformer remains a strong baseline
(\(0.904\)). A single STORM-Bz model is slightly lower (\(0.897\);
paired \(p = 0.038\)) but is significantly better at 1\,h
...
Transformer's multi-horizon PE while retaining the STORM 1-hour score.
The hybrid system routes the 1-hour head through a trained STORM-Bz model
and the 6\,h / 12\,h heads through the corresponding Transformer model at
inference time; no additional training is performed.
Against persistence at 1\,h, STORM remains near \(+0.32\) while the
Transformer is often negative (\(\approx-0.14\)).

Fig.~\ref{fig:seeds} shows the same pattern seed-by-seed rather than as a
single mean: the 1\,h STORM advantage is stable across initializations.
...
noise; the Transformer degrades sharply at 1\,h. Ensemble remains competitive
...
timing setup. STORM-Bz is lighter than the default Transformer baseline while remaining
competitive on all-hour 6\,h PE and improving short-horizon reliability. An architecture-matched
Transformer control ($d_{\mathrm{model}}=128$, 2 layers, 4 heads;
$\approx1.19\times10^{6}$ parameters) is reported separately in
Table~\ref{tab:main}.
...
\midrule
\(\approx\)1\,h & 0.820 $\pm$ 0.001 & 0.827 $\pm$ 0.009 & +0.007 \\
...
``winner'' architecture. At 1\,h, STORM improves reliability relative to a strong Transformer,
...
reached mean PE$_{1\mathrm{h}}=0.981$ and PE$_{6\mathrm{h}}=0.907$ across
fifteen seeds; bagging the fifteen matched seeds yields PE$_{1\mathrm{h}}=0.986$,
PE$_{6\mathrm{h}}=0.919$, and PE$_{12\mathrm{h}}=0.877$. Relative to STORM-Bz
($0.986$ / $0.897$ / $0.851$), matching encoder capacity shrinks the short-horizon
gap and shifts the multi-horizon advantage toward the larger Transformer under
bagging. This reinforces that single-model PE gaps are protocol-plus-capacity
...
remains below the GOES test scores, as expected under sensor and longitude shift. A similar adaptation profile is observed for the architecture-matched Transformer baseline, which improves its 6\,h PE from \(0.549 \pm 0.015\) (zero-shot) to \(0.625 \pm 0.021\) after fine-tuning.
...
Alternative gates, a hybrid SSM backbone, magnetopause features, and a spectral head were implemented and briefly evaluated; they are not used in the reported tables (\texttt{backbone=transformer}, spectral head off).
...
against persistence at 1\,h). The clearest multi-horizon gain among the systems evaluated here comes from an architecture-matched bagged Transformer, which reaches test PE\(_{6\mathrm{h}}=0.919\) and PE\(_{12\mathrm{h}}=0.877\). A linear ensemble with validation-selected \(\alpha^*=0.3\) and seed-bagged STORM also achieve strong multi-horizon results (both reaching PE\(_{6\mathrm{h}}=0.911\)). Input-noise and missing-channel checks, together with GRASP fine-tuning
gains at 6--12\,h, support operational motivation beyond a pure leaderboard PE
number; for instance, the architecture-matched baseline achieves a marginal persistence prediction efficiency (PE\(_{\mathrm{pers}}\)) of \(0.053 \pm 0.148\) at 1\,h, illustrating the difficulty of solidly outperforming operational persistence models. Furthermore, under synthetic Gaussian noise injection (\(\sigma=1.0\)), the 6\,h PE of the matched baseline drops to \(0.825 \pm 0.011\), highlighting the need for structurally robust architectures during degraded sensor conditions. Horizon-restricted physics losses did not improve PE in a fifteen-seed
ablation.
...
available at
\url{https://github.com/bnsama29-cloud/STORM-PhysNet}.
