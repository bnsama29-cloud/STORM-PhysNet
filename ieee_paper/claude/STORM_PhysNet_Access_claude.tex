...
On hourly GOES--OMNI data with fifteen random initializations of the same chronological split, STORM-Bz improves short-horizon reliability (PE$_{1\mathrm{h}}=0.986$ versus Transformer $0.977$, paired $p = 0.002$) and remains clearly positive against persistence (PE$\approx0.32$ versus $\approx-0.14$). At 6\,h and 12\,h a plain Transformer remains competitive or slightly stronger as a single model. A validation-selected linear ensemble ($\alpha^*=0.3$) and seed-bagged STORM both reach 6\,h PE of $0.911$, the strongest multi-horizon results among the systems evaluated in this study. A bagged Transformer control was not run; the gain from seed averaging is therefore not claimed to be architecture-specific. Controlled ablations show that the short-horizon gain is produced by the overall training protocol rather than by any single physics module at inference.
...
1\,hutes. On GSAT-19 GRASP at Indian longitude, zero-shot transfer is
...
\label{eq:horizons}
\end{equation}
The 1.0\,h (1\,h) head targets an operationally actionable warning window.
...
Transformer at inference; because a bagged Transformer was not evaluated,
we do not attribute this gain solely to the STORM architecture.
...
Predicted delays are constrained to \([0.5, 1.5]\,\mathrm{h}\). Across the full test set the delay distribution shows a non-trivial fraction of windows concentrating near the upper bound (\(1.5\,\mathrm{h}\)), with the remainder spread across the interior of the constraint band. This saturation at the upper bound indicates the box constraint is active; we therefore treat the histogram as a constrained aggregate rather than evidence of an unconstrained recovered transit time, which limits the physical interpretability of the raw delay values. Future work could include robustness checks with a wider constraint band to determine if the saturation is an artifact of the bound or a genuine feature of the optimization landscape. Gate activations differ between storm windows (\(\mathrm{Dst} \le -50\,\mathrm{nT}\)) and quiet windows (Mann--Whitney \(U = 8.82\times10^6\), \(p = 1.2\times10^{-14}\)). Absolute activations remain in a narrow band, so the storm-related shift is statistically clear but modest in magnitude.
...
Because a bagged Transformer was not evaluated, we do not attribute this
gain solely to the STORM architecture.
...
No bagged-Transformer control was evaluated; therefore claims about seed-bagging refer only to the systems reported in Table~\ref{tab:main}.
...
against persistence at 1\,h). The clearest multi-horizon gain among the systems evaluated here comes from a linear
